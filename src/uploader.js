...
export function setupUploader(element) {
  ...
  function renderResult(data) {
    const passed =
      typeof data.result === "string" && data.result.toUpperCase() === "PASS";
    ...
    const rows = FIELD_ORDER.map((key) => {
      ...
      if (!f) return "";
      const isPass =
        typeof f.status === "string" && f.status.toUpperCase() === "PASS";
      const chipClass = isPass ? "pass" : "fail";
      const chipLabel = isPass ? "PASS" : "FAIL";
      const label = FIELD_LABELS[key] || key;
      const found = f.found
        ? `<span class="result-found">${f.found}</span>`
        : "";
      const reason =
        !isPass && f.reason
          ? `<div class="result-reason">${f.reason}</div>`
          : "";
      return `<tr>
